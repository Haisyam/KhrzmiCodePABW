  ...
  <body class="h-full bg-white text-gray-900 antialiased">
    ...
    <main> 
      ...
      <section class="mx-auto max-w-[1120px] px-4 py-5">
        <div
          id="produks"
          class="grid gap-6 pt-4 sm:grid-cols-2 xl:grid-cols-4"
        ></div>
      </section>
      ...
      <div class="mx-auto max-w-[1120px] px-4 py-10">
        ...
        >
          ...
          >
            ...
            <a href="#produk" class="transition hover:text-black-500"
              >Produk</a
            >
            <span class="hidden h-3 w-px bg-gray-200 md:inline"></span>
    ...
    <script>
      async function loadProduks() {
        try {
          const response = await fetch("produks.json");
          if (!response.ok) {
            throw new Error("error menampilkan produk");
          }
      ...
      function renderProduks(produks) {
        const grid = document.getElementById("produks");
        if (!grid) return;

        const cardTemplate = (produk) => {
          const tagsMarkup = (produk.tags || [])
            .map(
              (tag) =>
                `<span class="inline-flex items-center rounded-full border border-gray-200 bg-gray-50 px-2.5 py-0.5 text-[11px] font-medium text-gray-600">${tag}</span>`
            )
            .join("");
          ...
          <a href="${
            produk.link || "#"
          }" class="group block rounded-3xl bg-white shadow-sm ring-1 ring-black/5 transition hover:-translate-y-1 hover:shadow-md">
            <div class="relative overflow-hidden rounded-xl rounded-b-none">
              <img src="${produk.image}" alt="${
            produk.name
          }" class="h-48 w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" />
              <div class="absolute inset-x-0 top-0 flex items-start justify-end px-4 pt-4 text-xs font-semibold">
            ...
            <div class="space-y-4 px-4 py-4">
              <div class="space-y-3">
                <span class="text-xs font-semibold uppercase tracking-[0.2em] text-gray-500">${
                  produk.category
                }</span>
              ...
              <div class="flex items-baseline gap-3">
                <span class="text-xl md:text-lg font-semibold text-black-500">${
                  produk.harga
                }</span>
              </div>
            </div>
        ...
        grid.innerHTML = produks.map(cardTemplate).join("");
      }
